...
func (h *Handler) resourceBanned(data []byte) error {
	...
	// To prevent double-refunding in case of redelivered messages without a
	// database transaction, we add an idempotency check (NOT EXISTS).
	refundRes, err := db.ExecContext(ctx, `
		INSERT INTO vault.tx_log (user_id, resource_id, balance, op_type)
		SELECT p.user_id, p.resource_id, p.amount, ?
		FROM vault.pledge p
		WHERE p.resource_id = ? AND p.funded = TRUE
		  AND NOT EXISTS (
		      SELECT 1 FROM vault.tx_log t
		      WHERE t.user_id = p.user_id
		        AND t.resource_id = p.resource_id
		        AND t.op_type = ?
		  )
	`, vaultModels.OpTypeAbuseRefund, m.Infohash, vaultModels.OpTypeAbuseRefund)
	if err != nil {
	...
	if _, err := db.ExecContext(ctx, `DELETE FROM vault.pledge WHERE resource_id = ?`, m.Infohash); err != nil {
		return errors.Wrap(err, "failed to delete vault.pledge rows")
	}
	if _, err := db.ExecContext(ctx, `DELETE FROM vault.resource WHERE resource_id = ?`, m.Infohash); err != nil {
		return errors.Wrap(err, "failed to delete vault.resource row")
	}

	// media_info cascades to movie, series (→ episode), episode.
	if _, err := db.ExecContext(ctx, `DELETE FROM media_info WHERE resource_id = ?`, m.Infohash); err != nil {
	...
	for _, q := range []string{
		...
		if _, err := db.ExecContext(ctx, q, m.Infohash); err != nil {
