...
*   **Bug 192 (Plaintext API Token Caching):** Modified Torbox `ResolveLink` backend to hash the plaintext user API token using SHA256 before cache-key formatting, preventing credentials from being stored in plaintext inside the Redis cache keys.
*   **AI Recommendations Prompt Injection Prevention:** Implemented a regex-based prompt injection detection engine in `web-ui/services/recommendations/prompt.go` to intercept and reject instruction hijack attempts in user queries, preventing LLM system prompt disclosure or service abuse.
