...
func (s *TorrentMap) Drop(h string) {
	s.mux.Lock()
	ti, ok := s.timers[h]
	if ok {
		ti.timer.Stop()
	...
	cl, err := s.tc.Get()
	if err != nil {
		log.WithError(err).WithField("infohash", h).Warn("failed to get torrent client for drop")
		return
	}
	t, ok := cl.Torrent(metainfo.NewHashFromHex(h))
	if ok {
		t.Drop()
...
func (s *TorrentMap) Get(ctx context.Context, h string) (*torrent.Torrent, error) {
	...
	if ok {
		...
		s.timers[h] = ti
		go func(h string, ti *torrentTimer) {
			select {
			case <-ti.timer.C:
			case <-ti.done:
				return
