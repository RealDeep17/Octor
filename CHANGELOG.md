...
- **Seeder SSD Cache Piece Thrashing & Concurrency Eviction Race**: Fully resolved the root cause of the SHA-1 mismatch crash loop during active ingestion stream operations. Implemented a thread-safe active `evicted` map inside `mmapTorrentStorage` protected under a dedicated RWMutex, fully integrated with the sharded per-piece RWMutex lock hierarchy to serialize page-reads, evictions, and refilling Writes. `ReadAt` now safely short-circuits on explicitly evicted blocks by returning a custom `errPieceEvicted` (triggering anacrolix re-verification and clean sequential download retry) while permitting ongoing verification reads to succeed. Audited and corrected piece index boundary calculations inside `piece_completion.go` and `mmap.go` to use `(offset + length - 1) / pieceLength` to prevent incorrect file completion invalidation on exact piece boundaries. Allows tight budgets (like the 1.5GB limit) to be respected completely under extreme peer speeds without any corruption or data leaks.
