...
const (
	...
	FlagGoogleSearch          = "ai-recommendations-google-search"
)

// Default models for different tiers.
const (
	DefaultModel      = "gemini-3.1-flash-lite"   // Legacy/Fallback default
	DefaultFreeModel  = "gemini-3.1-flash-lite"   // High RPM, balanced for free tier
	DefaultPaidModel  = "gemini-3.1-flash-lite"    // Fallback because Gemma doesn't support tools natively
)
...
func RegisterFlags(f []cli.Flag) []cli.Flag {
	return append(f,
		...
		cli.StringFlag{
			...
			Usage:  "Gemini API key (required when AI recommendations are enabled)",
			EnvVar: "GEMINI_API_KEY",
		},
		cli.StringFlag{
			Name:   FlagModel,
			Usage:  "Gemini model id (legacy — used as fallback for both tiers when free/paid overrides are unset)",
			Value:  DefaultModel,
			EnvVar: "AI_RECOMMENDATIONS_MODEL",
		},
		cli.StringFlag{
			Name:   FlagFreeModel,
			Usage:  "Gemini model id for free-tier users (overrides --ai-recommendations-model)",
			Value:  DefaultFreeModel,
			EnvVar: "AI_RECOMMENDATIONS_FREE_MODEL",
		},
		cli.StringFlag{
			Name:   FlagPaidModel,
			Usage:  "Gemini model id for paid-tier users (overrides --ai-recommendations-model)",
			Value:  DefaultPaidModel,
			EnvVar: "AI_RECOMMENDATIONS_PAID_MODEL",
		},
		cli.StringFlag{
			Name:   FlagChipsModel,
			Usage:  "Gemini model id for chip generation (defaults to free-tier model if unset — chips are lightweight and don't benefit from a smarter model)",
		...
		cli.IntFlag{
			...
			Usage:  "minimum release year for fresh releases injected into the AI prompt (compensates for Gemini's training cutoff)",
			Value:  2025,
		...
		cli.BoolFlag{
			Name:   FlagGoogleSearch,
			Usage:  "enable Google Search grounding for AI recommendations",
			EnvVar: "AI_RECOMMENDATIONS_GOOGLE_SEARCH",
		},
	)
}
...
func ConfigFromCLI(c *cli.Context) Config {
	return Config{
		...
		GeminiAPIKey:    c.String(FlagGeminiAPIKey),
		...
		GoogleSearch:          c.Bool(FlagGoogleSearch),
	}
}

// ResolveChipsModel returns the Gemini model id used for chip generation.
...
// falls back to the free-tier model (Flash), not the Pro one.
...
// ResolveModel returns the effective Gemini model id for the given tier,
