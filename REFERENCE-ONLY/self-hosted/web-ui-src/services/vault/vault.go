...
func RegisterFlags(f []cli.Flag) []cli.Flag {
	return append(f,
		cli.DurationFlag{
			...
			Value:  0, // self-hosted: no freeze, pledges can always be removed
...
// GetVaultAPIResource in self-hosted mode: no external vault storage API.
// Always return nil so statusLoop uses torrent-web-seeder stats instead of
// polling the sidecar (which doesn't implement the Vault API).
...
func (s *Vault) MarkResourceVaulted(ctx context.Context, resourceID string) error {
	db := s.pg.Get()
	if db == nil {
		return errors.New("database connection is not available")
	}
	return vaultModels.UpdateResourceVaulted(ctx, db, resourceID)
}
...
// putResourceToVaultAPI in self-hosted mode: no external vault storage API.
// Return false (not vaulted yet) so the resource stays in "Loading" state
// and the vault dashboard SSE shows real download progress (speed/ETA/peers).
func (s *Vault) putResourceToVaultAPI(ctx context.Context, tx *pg.Tx, resource *vaultModels.Resource) (bool, error) {
	log.WithField("resource_id", resource.ResourceID).
		Info("self-hosted mode: resource funded, waiting for download via torrent-web-seeder")
	return false, nil
}
