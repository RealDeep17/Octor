...
// ResetResourceVaulted resets the vaulted status for a resource
func ResetResourceVaulted(ctx context.Context, db pg.DBI, resourceID string) error {
	_, err := db.Model(&Resource{}).
		Context(ctx).
		Set("vaulted = false").
		Set("vaulted_at = NULL").
		...
		Update()
	if err != nil {
		return errors.Wrap(err, "failed to reset resource vaulted status")
	}
	return nil
}
