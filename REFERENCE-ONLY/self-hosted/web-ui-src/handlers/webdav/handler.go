...
func RegisterHandler(c *cli.Context, r *gin.Engine, pg *cs.PG, at *at.AccessToken, sapi *api.Api, jobs *j.Jobs) {
	...
	fs := NewFileSystem(pg, sapi, jobs, "webdav/fs")
...
func (s *Handler) handleWebDAV(c *gin.Context) {
	...
	// When arriving via a /s/{code} alias proxy, RequestURI still holds the
	// alias path (e.g. /s/j43gz1/) but url_alias has already rewritten
	// c.Request.URL.Path to the canonical internal path (e.g.
	// /token/{token}/webdav/fs/). Use URL.Path so PrefixDirectory can find
	// "webdav" in the path and strip the token prefix correctly.
	// For direct /webdav/fs/... requests, URL.Path is also already correct.
