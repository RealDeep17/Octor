...
func (s *WebSeeder) purgeCache(w http.ResponseWriter, h string) {
	if s.v != nil {
		s.v.Drop(h)
	}
	s.tm.Drop(h)
	s.tom.DropTouch(h)
	if err := s.fcm.Purge(h); err != nil {
		log.WithError(err).WithField("hash", h).Error("failed to purge file cache")
		http.Error(w, "failed to purge file cache", http.StatusInternalServerError)
		return
	}
