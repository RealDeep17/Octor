...
// Drop removes cached vault availability for a resource. This is used after
// vault deletion so stale 60s HEAD/GET results do not keep content looking
// vaulted after the user removes it.
func (s *Vault) Drop(hash string) {
	if s == nil {
		return
	}
	s.rootCache.Drop(hash)
	prefix := hash + "/"
	for _, key := range s.fileCache.Keys() {
		if strings.HasPrefix(key, prefix) {
			s.fileCache.Drop(key)
		}
	}
}
